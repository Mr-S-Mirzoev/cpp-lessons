...
int main(int argc, char const* argv[])
{
    ...
    // All arrays have 0 as their lower bound, so v has six elements, v[0] to
    // v[5]. The size of an array must be a constant expression. A pointer
    // variable can hold the address of an object of the appropriate type:
    char* p = &v_[3]; // p points to v's fourth element
    char x = *p;      // *p is the object that p points to

    // In an expression, prefix unary * means "contents of" and prefix unary &
    // means "address of." We can represent the result of that initialized
    ...
    // This for-statement can be read as "set i to zero; while i is not 10, copy
    // the i-th element and increment i." When applied to an integer variable,
    // the increment operator,
    // ++, simply adds 1. C++ also offers a simpler for-statement, called a
    // range-for-statement, for loops that traverse a sequence in the simplest
    // way:
    print();
    ...
    T& ref_ =
        a_[1]; // T&: reference to T. Cannot be uninitialized upon declaration.
    ...
    std::cout << "NO of occ of 'a' in \"AbraCaDabrA\" "
              << count_x("AbraCaDabrA", 'a') << std::endl;
    std::cout << "NO of occ of 'a' in nullptr " << count_x(nullptr, 'a')
              << std::endl;

    char* ptr;
    std::cout << "NO of occ of 'a' in uninitialized string "
              << count_x(ptr, 'a') << std::endl;
